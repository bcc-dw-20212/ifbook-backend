...
@click.group()
def manage():
    """This script manages the ifbookbackend project."""
...
def runserver(port):
    ...
    cmd = ""
    if os.name == "posix":
        cmd = f"export FLASK_APP=ifbookbackend.app; export FLASK_ENV=development; flask run --port={port}"
        subprocess.run(cmd, shell=True)
    elif os.name == "nt":
        cmd = f'$env:FLASK_APP = "ifbookbackend.app"; $env:FLASK_ENV = "development"; flask run --port={port}'
...
def plug_extension(name: str):
    ...
    with open(
        os.path.join(os.getcwd(), "ifbookbackend", "ext", f"{name}.py"), "w"
    ) as new_extension:
...
def plug_blueprint(name: str, templates: bool):
    ...
    settings = toml.load(os.path.join(os.getcwd(), "instance", "settings.toml"))
    if (
        f"ifbookbackend.blueprints.{name}.{name}:init_app"
        in settings["default"]["EXTENSIONS"]
    ):
        click.echo("A blueprint with such a name seems to be already plugged.")
        exit(0)
    os.mkdir(os.path.join(os.getcwd(), "ifbookbackend", "blueprints", name))
    if templates:
        tf = os.path.join(
            os.getcwd(), "ifbookbackend", "blueprints", name, "templates", name
        )
        os.makedirs(tf)
...
def plug_database():
    ...
    # project.ext.database
    with open(
        os.path.join(os.getcwd(), "ifbookbackend", "ext", "database.py"), "w"
    ) as db_module:
        db_template = get_template("database.pyt")
        db_module.write(db_template.render(name="ifbookbackend"))
    # models.py (basic example)
    with open(
        os.path.join(os.getcwd(), "ifbookbackend", "models.py"), "w"
    ) as models_file:
    ...
    cmd = ""
    if os.name == "posix":
        cmd = f"export FLASK_APP=ifbookbackend.app; export FLASK_ENV=development; flask db init"
        subprocess.run(cmd, shell=True)
    elif os.name == "nt":
        cmd = f'$env:FLASK_APP = "ifbookbackend.app"; $env:FLASK_ENV = "development"; flask db init'
        subprocess.run(["powershell", "-Command", cmd])
